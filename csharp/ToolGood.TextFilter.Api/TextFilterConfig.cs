...
namespace ToolGood.TextFilter.Api
{
    public class TextFilterConfig
    {
        ...
        public ImageFilterGrpcProvider CreateImageFilterGrpcClient()
        {
            ...
            return new ImageFilterGrpcProvider(GrpcHost);
        }

        public ImageFilterGrpcAsyncProvider CreateImageFilterGrpcAsyncClient()
        {
            if (isCreate == false) {
                AppContext.SetSwitch("System.Net.Http.SocketsHttpHandler.Http2UnencryptedSupport", true);
                isCreate = true;
            }
            return new ImageFilterGrpcAsyncProvider(GrpcHost);
        }
        ...
        public ImageClassifyGrpcProvider CreateImageClassifyGrpcClient()
        {
            ...
            return new ImageClassifyGrpcProvider(GrpcHost);
        }
        public ImageClassifyGrpcAsyncProvider CreateImageClassifyGrpcAsyncClient()
        {
            if (isCreate == false) {
                AppContext.SetSwitch("System.Net.Http.SocketsHttpHandler.Http2UnencryptedSupport", true);
                isCreate = true;
            }
            return new ImageClassifyGrpcAsyncProvider(GrpcHost);
        }
