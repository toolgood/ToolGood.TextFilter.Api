...
namespace ToolGood.TextFilter.Api.Interfaces
{
    public interface ISysProvider
    {
        ...
        /// <param name="imageFilterNoticeUrl">默认 图片检测异步地址</param>
        /// <param name="imageClassifyNoticeUrl">默认 图片分类异步地址</param>
        /// <param name="imageTempPath">图片临时保存地址</param>
        ...
        /// <param name="imageTempPath">图片临时保存地址</param>
        /// <param name="skipword">自定义跳词</param>
        Task<CommonResult> UpdateSystemAsync(string textFilterNoticeUrl, string textReplaceNoticeUrl, string imageFilterNoticeUrl
            , string imageClassifyNoticeUrl, string imageTempPath, string skipword);
        /// <summary>
        /// 刷新缓存
        /// </summary>
