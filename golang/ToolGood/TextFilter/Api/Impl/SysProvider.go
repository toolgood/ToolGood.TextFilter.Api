...
func (this *SysProvider)UpdateSystem(textFilterNoticeUrl string,textReplaceNoticeUrl string,imageFilterNoticeUrl string,imageClassifyNoticeUrl string,imageTempPath string,skipword string) *CommonResult  {
	u:=this.textFilterHost+updateSystemUrl;

	resp, err := http.PostForm(u, url.Values{ "textFilterNoticeUrl":{textFilterNoticeUrl}, "textReplaceNoticeUrl":{textReplaceNoticeUrl}, "imageFilterNoticeUrl":{imageFilterNoticeUrl},	"imageClassifyNoticeUrl":{imageClassifyNoticeUrl},"imageTempPath":{imageTempPath},"skipword":{skipword}	}) 
    if err != nil {
		return nil
    }
